...
UPDATES_CHANNEL = os.environ.get('UPDATES_CHANNEL', 'xewos')
...
async def genStr(_, msg: Message):
    if msg.chat.id in Credentials.BANNED_USERS:
        await bot.send_message(
            chat_id=msg.chat.id,
            text="You are Banned. [Contact Me](tg://user?id=5240498249)",
    ...
    if update_channel:
        try:
            ...
            if user.status == "kicked":
               await bot.send_message(
                   chat_id=msg.chat.id,
                   text="Sorry Sir, You are Banned. [Contact Me](tg://user?id=5240498249).",
                   parse_mode="markdown",
            ...
            await bot.send_message(
                chat_id=msg.chat.id,
                text="**Please Join Channel To Use Me!**",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton("▶️ Login To Channel", url=f"https://t.me/{update_channel}")
                        ]
                    ]
                ),
                parse_mode="markdown"
            ...
            await bot.send_message(
                chat_id=msg.chat.id,
                text="**Something Went Wrong.** [Contact Me](tg://user?id=5240498249).",
                parse_mode="markdown",
                disable_web_page_preview=True
            )
            return
    ...
    try:
        ...
        await client.send_message("me", f"#PYROGRAM #STRING_SESSION \n\n```{session_string}``` \n\nBy [String Session Generator](http://t.me/StringCreatorBot) 🤖\nMade with ❤️ By [Abolfazl](tg://user?id=5240498249)")
        await client.disconnect()
...
"""
    reply_markup = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton('CHANNEL', url='https://t.me/xewos'),
                InlineKeyboardButton('SUPPORT', url='tg://user?id=5240498249')
            ],
            [
                InlineKeyboardButton('DEVELOPER', url='tg://user?id=5240498249'),
            ]
        ]
    )
